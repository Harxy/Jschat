                ...
                request({
                    ...
                    if (!error && response.statusCode === 200) {
                        if (body.length > 0) {
                            ...
                            playQueue.queueItem( room, {data: {'service': "youtube", id: param, 'title': JSONbody.items[0].snippet.title}, duration: convert_time(JSONbody.items[0].contentDetails.duration)*1000, requester: personName});
                        }
                    }
                    callback(message);
                });
